...
const ManagementView: React.FC<ManagementViewProps> = ({ items, expandedObjectives, onToggleObjective }) => {
  ...
  // Get business objectives associated with each outcome
  const getBusinessObjectives = (outcomeId: string) => {
    return objectives.filter(obj => obj.outcome_id === outcomeId);
  };
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            {/* Business Objectives */}
                            {getBusinessObjectives(outcome.id).length > 0 && (
                              <div className="mt-2">
                                <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
                                  Business Objectives
                                </h4>
                                <ul className="space-y-1">
                                  {getBusinessObjectives(outcome.id).map(obj => (
                                    <li key={obj.id} className="flex items-center text-sm text-gray-700">
                                      <TargetIcon className="h-3 w-3 text-blue-500 mr-1.5 flex-shrink-0" />
                                      <span>{obj.title}</span>
                                    </li>
                                  ))}
                                </ul>
                            ...
                            {/* Metrics */}
                            {getOutcomeMetrics(outcome.id).length > 0 && (
                              <div className="mt-3 space-y-2">
                                ...
                                <div className="space-y-2">
                                  {getOutcomeMetrics(outcome.id).map((metric) => (
